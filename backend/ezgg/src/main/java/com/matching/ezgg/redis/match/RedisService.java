...
public class RedisService {
    ...
    public void saveMatchRequest(MatchingFilterParsingDto matchingFilterParsingDto) {
        try {
            ...
            // 기존 데이터 확인
            String existingStreamId = (String) redisTemplate.opsForHash().get(RedisKey.STREAM_ID_HASH_KEY.getValue(), memberId);
    ...
    public void acknowledgeMatch(Long memberId) {
        try {
            ...
            String streamId = (String) redisTemplate.opsForHash().get(RedisKey.STREAM_ID_HASH_KEY.getValue(), memberIdStr);

            if (streamId != null) {
                redisTemplate.opsForStream().acknowledge(RedisKey.STREAM_KEY.getValue(), RedisKey.STREAM_GROUP.getValue(), streamId);
                ...
                log.info("유저 Stream 처리 완료 (ACK & 삭제) : {}", memberIdStr);
            }
        } catch (Exception e) {
